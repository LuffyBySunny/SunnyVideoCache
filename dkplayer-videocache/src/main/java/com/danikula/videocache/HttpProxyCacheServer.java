...
public class HttpProxyCacheServer {
    ...
    private String appendToProxyUrl(String url) {
        return String.format(Locale.US, "http://%s:%d/preload?url=%s", PROXY_HOST, port, ProxyCacheUtils.encode(url));
    }
    ...
    public File getTempCacheFile(String url) {
        ...
        Log.d("getTempCacheFile", "generate--" + url + "---" + fileName);
        return new File(cacheDir, fileName);
    }
    ...
    private void processSocket(Socket socket) {
        try {
            ...
            String rawUrl = Uri.parse(request.uri).getQueryParameter("url");
            Log.d("url", url);
            HttpProxyCacheServerClients clients = getClients(rawUrl);
